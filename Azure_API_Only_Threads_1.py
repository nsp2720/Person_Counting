...
# Global dictionary to track last API call time for each camera
last_api_call_time = {}

# Function to upload the annotated frame to Azure Blob Storage and optionally send data to POST API
def upload_to_azure_and_post(frame, azure_connection_string, container_name, cameradid, post_api_url, an_id, person_count, send_to_api):
    try:
        ...
        # Send data to POST API if required
        if send_to_api:
            data = {
                "cameradid": cameradid,
                "sendtime": send_time,
                "imgurl": image_url,
                "an_id": an_id,
                "ImgCount": person_count
            }
            headers = {'Content-Type': 'application/json'}
            response = requests.post(post_api_url, json=data, headers=headers)
            if response.status_code == 200:
                print(f"Data sent to POST API successfully for {cameradid}: {response.json()}")
            else:
                print(f"POST API error for {cameradid}: {response.status_code}, {response.text}")
    except Exception as e:
        print(f"Error uploading frame or sending data to POST API for {cameradid}: {e}")
...
def process_stream(rtmp_url, azure_connection_string, container_name, post_api_url, an_id, model):
    ...
    frame_count = 0
    last_api_call_time[cameradid] = datetime.min  # Initialize last API call time
    ...
    while True:
        ...
        # Display person count in the top-middle of the frame
        text = f"Person Count: {person_count}"
        (w, h), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
        position = ((frame.shape[1] - w) // 2, 50)
        cv2.putText(frame, text, position, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)

        # Check if 60 seconds have passed since the last API call
        current_time = datetime.now()
        time_since_last_api_call = (current_time - last_api_call_time[cameradid]).total_seconds()

        # Upload frame to Azure Blob Storage and send to API if necessary
        send_to_api = time_since_last_api_call >= 60
        if send_to_api:
            last_api_call_time[cameradid] = current_time
        
        threading.Thread(
            target=upload_to_azure_and_post,
            args=(frame, azure_connection_string, container_name, cameradid, post_api_url, an_id, person_count, send_to_api),
            daemon=True
        ).start()
